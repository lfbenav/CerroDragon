...
export default function TourInfoPage() {
    ...
    // Cargar datos del tour y paquetes desde la API
    useEffect(() => {
    ...
    const confirmDelete = async () => {
        if (paqueteToDelete === null) return;
        
        const paquete = paquetes[paqueteToDelete];
        try {
            const token = localStorage.getItem('access_token');
            const res = await fetch(`${API_URL}/tour-packages/${paquete.id}`, {
                method: 'DELETE',
                headers: {
            ...
            if (res.ok) {
                setPaquetes(prev => prev.filter((_, i) => i !== paqueteToDelete));
            }
        } catch (error) {
    ...
    const handleEditPaquete = (index: number) => {
        ...
        router.push(`/admin/tours/editar-paquete?tourId=${id}&paqueteId=${paquete.id}`);
    };
