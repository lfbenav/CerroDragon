...
export default function NuevoComidaFormPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${API_URL}/foods/comidas`, {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          option_name: nombre.trim(),
        }),
      });

      if (!response.ok) {
        throw new Error(`Error al crear la comida (${response.status})`);
      }

      const json = (await response.json()) as CreateMealResponse;

      if (!json.success) {
        throw new Error(json.message || "Error al crear la comida");
      }
